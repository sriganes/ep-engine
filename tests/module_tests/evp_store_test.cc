...
// which only calls notify_IO_complete and isn't involved with any
...
// there is no guarantee that we will actually deadlock on any given run.
TEST_F(EventuallyPersistentStoreTest, test_mb20751_deadlock_on_disconnect_delete) {
    ...
    // 1. To check that there is no potential data race with the
